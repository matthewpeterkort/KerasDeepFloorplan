...
#Read in a multi image, get the number of the image from the image path. This number is used to pair the Multi image with the close wall image label for it's respective image  
...
#pass in the label, stitch together the path where the wall that pairs with the multi resides. Load the wall label image, and write it to the TF record as a bytestring feature. 
def to_tfrecord(img_bytes,label):
	...
	#This code block was used to invert the training data label images so that the black wall white background is the same in both the base image and the label image.
...
def main():
		...
		nb_images=len(tf.io.gfile.glob('C:/Users/matth/Desktop/Ok/cubicasa5k/cubicasa5k/collected/*.png'))
		...
		dataset = tf.data.Dataset.list_files('C:/Users/matth/Desktop/Ok/cubicasa5k/cubicasa5k/collected/*.png',shuffle=True)
		...
		model = tf.keras.Sequential([
			...
			tf.keras.layers.Conv2DTranspose(filters=256,kernel_size=4, strides=2, padding='same',activation='relu'),
		...
		#model.compile(optimizer='Adam',loss=tf.keras.losses.BinaryFocalCrossentropy(from_logits=False),metrics=tf.keras.metrics.BinaryAccuracy(threshold=0.5))
		#model.fit(get_training_dataset(training_filenames),steps_per_epoch=350,validation_steps=75, epochs=5,validation_data=get_validation_dataset(validation_filenames))
		#model.save("NEWWINDOWANDDOORNET")
		...
		newEST_MODEL=load_model("NEWWINDOWANDDOORNET")
		#newEST_MODEL.name="new_model_yellow"
		...
		Door_models= [model_Walldoormemes,model_DOORWINDOWNETONE,model_DOORWINDOWNETTWO,newEST_MODEL]
		...
		ds = get_validation_dataset(filenames)
		#newEST_MODEL.evaluate(ds,steps=50)
		...
		#Wall_ensemble_model.evaluate(ds,steps=50)
		...
		for images in ds.take(10):
			...
			cnt = 1
			for img, prediction ,prediction_two in zip(images, predicted_classes,predicted_classes_two ):
				...
				plt.imsave(filename_wall,prediction_two,cmap='binary')
				...
				plt.text(.83,.02,s='base image '+str(int((cnt+1)/2)), horizontalalignment='center', transform=ax.transAxes)
				...
				bits[np.all(bits == (191,255,128), axis=-1)] = (48,23,16)  # change the background color to white
				bits[np.all(bits == (128,255,191), axis=-1)] = (48,23,16)
				
				bits[np.all(bits == (64,128,0), axis=-1)] = (255,255,255)   # change the Wall color to black
				bits[np.all(bits == (0,128,64), axis=-1)] = (255,255,255)
				...
				cnt = cnt + 1
				ax = fig.add_subplot(3, 4, cnt)
				plt.text(.83,.02,s='prediction '+str(int(cnt/2)), horizontalalignment='center', transform=ax.transAxes)
				plt.imshow(bits)			
